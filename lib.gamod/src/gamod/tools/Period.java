...
public final class Period {
  ...
  static {
    int periodOfC2 = 42800;
    int indexOfC2 = (3 * tonesPerOctave + 1) * stepsPerTone;
    double stepsPerOctave = tonesPerOctave * stepsPerTone;
    for (int i = 0; i < periods.length; i++)
      periods[i] = (int)round(periodOfC2 * pow(2.0, (indexOfC2 - i) / stepsPerOctave));
  ...
  public static int getPeriodForKey(int key, int finetune) {
  ...
  public static int getKeyForPeriod(int period) { // table lookup
    if (period == 0)
      return 0;
  ...
  public static String getKeyName(int key, boolean hold) {
    return key == 0 && hold ? " * " : key < 0 || key >= keyNames.length ? " ! " : keyNames[key];
  }

  public static int snapPeriod(int period) {
    return tonePeriods[getKeyForPeriod(period)];
  }

  public static int cropPeriod(int period) {
    return crop(period, periods[periods.length - 1], periods[0]);
  }

  public static long getSamplingStepForPeriod(int sampleFrequence, boolean pal) {
    long ratePal_x100 = 354689460l;  // PAL  (50Hz): rate = 3546894.6
    long rateNtsc_x100 = 357954525l; // NTSC (60Hz): rate = 3579545.25
    // Hz = rate / amigaPeriod,
